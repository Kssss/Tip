...
#import "MTSSLog.h"
    ...
    #define DEVDBAutorelease(__v) ([__v autorelease]);
    #define DEVDBReturnAutoreleased DEVDBAutorelease

    #define DEVDBRetain(__v) ([__v retain]);
    #define DEVDBReturnRetained DEVDBRetain

    #define DEVDBRelease(__v) ([__v release]);

    #define DEVDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define DEVDBAutorelease(__v)
    #define DEVDBReturnAutoreleased(__v) (__v)

    #define DEVDBRetain(__v)
    #define DEVDBReturnRetained(__v) (__v)

    #define DEVDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define DEVDBDispatchQueueRelease(__v)
    #else
        #define DEVDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^DEVDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);


/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in DEVDB are:

 - `WXGZDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<WXGZResultSet>` - Represents the results of executing a query on an `WXGZDatabase`.
 - `<WXGZDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<WXGZDatabasePool>` - A pool of `WXGZDatabase` objects.
 - `<DEVStatementSDK>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [DEVDB on GitHub](https://github.com/ccgus/DEVdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [DEVDB mailing list](http://groups.google.com/group/DEVdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `WXGZDatabase` object and use it across multiple threads. Instead, use `<WXGZDatabaseQueue>`.
...
@interface WXGZDatabase : NSObject  {
...
/** Create a `WXGZDatabase` object.
 
 An `WXGZDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `WXGZDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `WXGZDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    WXGZDatabase *db = [WXGZDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    WXGZDatabase *db     = [WXGZDatabase databaseWithPath:dbPath];
 ...
 @return `WXGZDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `WXGZDatabase` object.
...
/** Execute single update statement
 ...
 @note If you want to use this from Swift, please note that you must include `WXGZDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeUpdate:withArgumentsInArray:>`.
 */
...
/** Execute single update statement
 ...
 This method executes a single SQL update statement (i.e. any SQL that does not return results, such as `UPDATE`, `INSERT`, or `DELETE`. This method employs [`sqlite3_prepare_v2`](http://sqlite.org/c3ref/prepare.html) and [`sqlite_step`](http://sqlite.org/c3ref/step.html) to perform the update. Unlike the other `executeUpdate` methods, this uses printf-style formatters (e.g. `%s`, `%ld`, etc.) to build the SQL. Do not use `?` placeholders in the SQL if you use this method.
...
/** Execute single update statement
 ...
 This method executes a single SQL update statement (i.e. any SQL that does not return results, such as `UPDATE`, `INSERT`, or `DELETE`. This method employs [`sqlite3_prepare_v2`](http://sqlite.org/c3ref/prepare.html) and [`sqlite_step`](http://sqlite.org/c3ref/step.html) to perform the update. Unlike the other `executeUpdate` methods, this uses printf-style formatters (e.g. `%s`, `%ld`, etc.) to build the SQL.
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(DEVDBExecuteStatementsCallbackBlock)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<WXGZResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[WXGZResultSet next]>`) from one record to the other.
 ...
 @return A `<WXGZResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see WXGZResultSet
 @see [`WXGZResultSet next`](<[WXGZResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 
 @note If you want to use this from Swift, please note that you must include `WXGZDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeQuery:withArgumentsInArray:>`.
 */

- (WXGZResultSet *)executeQuery:(NSString*)sql, ...;
...
- (WXGZResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (WXGZResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (WXGZResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (WXGZResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;

/**
 *  @author Rain, 15-11-03 20:11:03
 *
 *  @brief  将.m  类别中的方法 移到.h,供外外部使用.
 *
 *  @return 是否执行成功.
 */
// TODO Private api need by AFDBService
 ...
 *  @return 是否执行成功.
 */
- (WXGZResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(WXGZDatabase *adb) {
        ...
        [adb makeFunctionNamed:@"StringStartsWithH" maximumArguments:1 withBlock:^(sqlite3_context *context, int aargc, sqlite3_value **aargv) {
            if (sqlite3_value_type(aargv[0]) == SQLITE_TEXT) {
                ...
                MTSSLog(@"Unknown formart for StringStartsWithH (%ld) %s:%ld", sqlite3_value_type(aargv[0]), __FUNCTION__, __LINE__);
                sqlite3_result_null(context);
            }
        }];

        int rowCount = 0;
        WXGZResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            MTSSLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
        DEVDBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [WXGZDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one DEVDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using WXGZDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to DEVDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using DEVDB you will not need to interact directly with `DEVStatementSDK`, but rather with `<WXGZDatabase>` and `<WXGZResultSet>` only.
...
@interface DEVStatementSDK : NSObject {
