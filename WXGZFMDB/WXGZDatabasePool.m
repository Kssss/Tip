...
@interface WXGZDatabasePool()

- (void)pushDatabaseBackInPool:(WXGZDatabase*)db;
- (WXGZDatabase*)db;

@end


@implementation WXGZDatabasePool
...
+ (instancetype)databasePoolWithPath:(NSString*)aPath flags:(int)openFlags {
    return DEVDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"DEVdb.%@", self] UTF8String], NULL);
        _databaseInPool     = DEVDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = DEVDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    DEVDBRelease(_path);
    DEVDBRelease(_databaseInPool);
    DEVDBRelease(_databaseOutPool);
    
    if (_lockQueue) {
        DEVDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(WXGZDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The WXGZDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (WXGZDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            if (self->_maximumNumberOfDatabasesToCreate) {
                ...
                if (currentCount >= self->_maximumNumberOfDatabasesToCreate) {
                    MTSSLog(@"Maximum number of databases (%ld) has already been reached!", (long)currentCount);
                    return;
                }
            }
            
            db = [WXGZDatabase databaseWithPath:self->_path];
        ...
        if (success) {
            ...
            MTSSLog(@"Could not open up the database at path %@", self->_path);
            db = 0x00;
        }
    }];
