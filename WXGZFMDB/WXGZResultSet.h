...
@class WXGZDatabase;
@class DEVStatementSDK;

/** Represents the results of executing a query on an `<WXGZDatabase>`.
...
@interface WXGZResultSet : NSObject {
    WXGZDatabase          *_parentDB;
    DEVStatementSDK         *_statement;
...
/** `DEVStatementSDK` used by result set. */

@property (atomic, retain) DEVStatementSDK *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<DEVStatementSDK>`
 
 @param statement A `<DEVStatementSDK>` to be performed
 
 @param aDB A `<WXGZDatabase>` to be used
 
 @return A `WXGZResultSet` on success; `nil` on failure
 */
