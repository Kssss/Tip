...
@implementation WXGZResultSet
@synthesize query=_query;
@synthesize statement=_statement;

+ (instancetype)resultSetWithStatement:(DEVStatementSDK *)statement usingParentDatabase:(WXGZDatabase*)aDB {
    
    WXGZResultSet *rs = [[WXGZResultSet alloc] init];
    ...
    return DEVDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    DEVDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return DEVDBReturnAutoreleased([dict copy]);
    }
    else {
        MTSSLog(@"Warning: There seem to be no columns in this set.");
    }
...
//        MTSSLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
//        MTSSLog(@"Database busy");
        if (outErr) {
...
//        MTSSLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
...
//        MTSSLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"WXGZDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
        ...
        // wtf?
//        MTSSLog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    MTSSLog(@"Warning: I could not find the column named '%@'.", columnName);
...
- (void)setParentDB:(WXGZDatabase *)newDb {
    _parentDB = newDb;
}
